...
public class Http {
    ...
    public void mainHandler(APIGatewayProxyRequestEvent apiGatewayProxyRequestEvent,Context context) {
        ...
        //更改为自己的账号密码
        map.put("phone_number", "password");
        Set<String> strings = map.keySet();
        for (String string : strings) {
            try {
                String s = map.get(string);
                String accessCode = getAccessCode(string, s);
                ...
                String time = getTime();
                updateStep(appToken, user_id,time);
            } catch (Exception e) {
    ...
    public  void updateStep(String appToken, String userId,String time) {
        Date date = new Date();
        ...
        Integer step = 9456;
        int integer = Integer.parseInt(hour)+8;
        if (integer == 10) {
            step = new Random().nextInt(1999)+8000;
        } else if (integer == 11) {
            step = new Random().nextInt(1999)+9000;
        } else if (integer >= 12) {
            step = new Random().nextInt(1999)+18000;
        } else {
            step = new Random().nextInt(1999)+18000;
        }
